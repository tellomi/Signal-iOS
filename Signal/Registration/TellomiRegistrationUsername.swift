...
public enum TellomiRegistrationUsername {
    ...
    ///
    /// 候选只留原名里的字母和数字，`_` 一个不留（中间的也去掉），截到 17 位（taishi 审查包 4、中转包 8，Android 同一刀）：
    /// 词库的 PREFIX 规则以「非字母数字」为边界，`kefu_58`、`tellomi_support27` 正是要拦的形状，而服务端拒绝表只收
    /// 「保留词 × 01–99」，这种候选能保留成功。紧跟数字不算边界，`kefu27` 不命中。
    static func candidates<R: RandomNumberGenerator>(for nickname: String, count: Int = 3, using random: inout R) -> [String] {
        let usable = String(nickname.lowercased().unicodeScalars.filter { isAsciiLetter($0) || isAsciiDigit($0) }.map(Character.init))
        let base = String(usable.prefix(maxLength - 3))
